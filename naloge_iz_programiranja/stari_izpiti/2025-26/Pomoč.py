...
"""
for povezava, povezane_postaje in povezave42.items():
    print("ime postaje:", povezava)
    print("ime povezane postaje:", povezane_postaje)
    print("koliko je dolg seznam:", len(povezane_postaje))
    imena_postaj = set()
    for ime_postaje, _, _ in povezane_postaje:
        imena_postaj.add(ime_postaje)
        print(len(imena_postaj))
...
print(potovalni_cas(povezave42, ["Kranj", "Škofja Loka", "Ljubljana", "Litija", "Hrastnik"], 420))
"""
...
def vrh(visine, x, y):
    i = 0 #stolpec
    j = 0 #vrstica
    for zgornja, spodnja in zip(visine, visine[1:]):
        for gor, dol in zip(zgornja, spodnja):
            if i == x and j == y:
                print(f"jaz sem {gor}")
                print(f"{gor} je v {i} stolpcu in {j} vrstici")
                print(f"nad mano je {zgornja[y + 1]}")
            i += 1
        j += 1

print(vrh(visine, 2, 3))