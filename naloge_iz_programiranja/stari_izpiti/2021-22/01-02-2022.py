...
def ogrozena(povezave):
    ...
    for kraj1, kraj2 in povezave:
            slovar[kraj1] += 1
            slovar[kraj2] += 1
    return set([ime_kraja for ime_kraja in slovar if slovar[ime_kraja] == 1])
...
def zasede(poti, povezave):
    zasedene_povezave = set()
    vse_povezave = [frozenset(povezava) for povezava in povezave]
    st_uspesni = 0

    for pot in poti:
        uspesna = True
        for kraj1, kraj2 in pairwise(pot):
            povezava = frozenset({kraj1, kraj2})
            if povezava not in vse_povezave or povezava in zasedene_povezave:
                uspesna = False
                break
            zasedene_povezave.add(povezava)
...
class Enota:
    def __init__(self, kraj_izkrcanja, povezave, visine):
        self.kraj_izkrcanja = kraj_izkrcanja
        self.povezave= povezave
        self.visine = visine
        self.trenutni_kraj = kraj_izkrcanja
        self.seznam_obiskanih = [kraj_izkrcanja]
        self.vsota_vzponov = 0

    def premik(self, kam):
        if (self.trenutni_kraj, kam) in self.povezave or (kam, self.trenutni_kraj) in self.povezave:
            razlika = self.visine[kam] - self.visine[self.trenutni_kraj]
            if razlika > 0:
                self.vsota_vzponov += razlika
            self.trenutni_kraj = kam
            self.seznam_obiskanih.append(kam)
            return True
        return False

    def pot(self):
        return self.seznam_obiskanih

    def vzpon(self):
        return self.vsota_vzponov
