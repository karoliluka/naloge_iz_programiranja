...
def postaje(povezave):
    ...
    for postaja, povezane_postaje in povezave.items():
        imena_postaj = set()
        for ime_postaje, _, _ in povezane_postaje:
            imena_postaj.add(ime_postaje)

        if len(imena_postaj) == 1:
            koncne.add(postaja)
        elif len(imena_postaj) == 2:
...
def naslednja_povezava(povezave, odkod, kam, cas):
    casi_odhodov = []
    for povezava, povezane_postaje in povezave.items():
        for ime_povezane_postaje, cas_odhoda_s_postaje, cas_prihoda_na_povezano in povezane_postaje:
            if povezava == odkod and kam == ime_povezane_postaje:
                casi_odhodov.append((cas_odhoda_s_postaje, cas_prihoda_na_povezano))

    kandidati = []
    for (cas_odhoda, cas_prihoda) in casi_odhodov:
        if cas == cas_odhoda:
            return cas_odhoda, cas_prihoda

        elif cas < cas_odhoda:
            kandidati.append((cas_odhoda, cas_prihoda))
...
def potovalni_cas(povezave, pot, zacetek):
    start = zacetek
    casi = []
    for ime_povezave, ime_ciljne_povezave in zip(pot, pot[1:]):
        if naslednja_povezava(povezave, ime_povezave, ime_ciljne_povezave, zacetek):
            cas_odhoda, cas_prihoda = naslednja_povezava(povezave, ime_povezave, ime_ciljne_povezave, zacetek)
            zacetek = cas_prihoda
            casi.append((cas_odhoda, cas_prihoda))
        else:
            return None
    return casi[-1][1] - start

def vozni_red(povezave, linija, ime_datoteke):
    vlaki = []
    for ime_naslednje_postaje, cas_odhoda, cas_prihoda in povezave[linija[0]]:
        if ime_naslednje_postaje == linija[1]:
            # to je en kandidat za vlak - začnemo mu graditi seznam časov
            casi_vlaka = [cas_odhoda, cas_prihoda]
            trenutna_postaja = linija[1]
            trenutni_cas = cas_prihoda
            veljaven_vlak = True

            # 2. korak: sledimo vlaku skozi preostale postaje na liniji
            for naslednja_postaja in linija[2:]:
                rezultat = naslednja_povezava(povezave, trenutna_postaja, naslednja_postaja, trenutni_cas)
                if rezultat is None:
                    veljaven_vlak = False
                    break
                cas_odhoda_naprej, cas_prihoda_naprej = rezultat
                casi_vlaka.append(cas_prihoda_naprej)
                trenutna_postaja = naslednja_postaja
                trenutni_cas = cas_prihoda_naprej

            if veljaven_vlak:
                vlaki.append(casi_vlaka)

    # 3. korak: uredimo vlake po času prvega odhoda
    vlaki.sort(key=lambda casi: casi[0])

    # 4. korak: izpišemo v datoteko, vrstica na postajo
    with open(ime_datoteke, "w", encoding="UTF-8") as datoteka:
        vrstice = []
        for i, postaja in enumerate(linija):
            vrstica = f"{postaja:>20}"
            for casi_vlaka in vlaki:
                cas = casi_vlaka[i]
                ure = cas // 60
                minute = cas % 60
                vrstica += f"  {ure:02d}:{minute:02d}"
            vrstice.append(vrstica)
        datoteka.write("\n".join(vrstice))


def cas_prihoda(povezave, odkod, kam, zacetek, omejitev):
    if odkod == kam:
        return zacetek

    najboljsi = None
    trenutna_omejitev = omejitev

    for ime_naslednje_postaje, cas_odhoda_s_postaje, cas_prihoda_na_postajo in povezave[odkod]:
        # vlak moramo ujeti (odpelje po našem trenutnem času) in
        # že samo prihod na vmesno postajo ne sme presegati najboljšega, kar smo doslej našli
        if cas_odhoda_s_postaje >= zacetek and cas_prihoda_na_postajo <= trenutna_omejitev:
            rezultat = cas_prihoda(povezave, ime_naslednje_postaje, kam, cas_prihoda_na_postajo, trenutna_omejitev)
            if rezultat is not None and (najboljsi is None or rezultat < najboljsi):
                najboljsi = rezultat
                trenutna_omejitev = rezultat  # zaostrimo mejo - iščemo samo še boljše poti
...
class Potnik:
    def __init__(self, zacetna_postaja, trenutni_cas):
        self.zacetna_postaja = zacetna_postaja
        self.trenutni_cas = trenutni_cas
        self.trenutna_postaja = zacetna_postaja
        self.skupni_cas_cakanja = 0

    #premik ni delal, saj nisem v moji verziji uporabljal zgornje funkcije naslednja_povezava
    def premik(self, kam):
        rezultat = naslednja_povezava(povezave, self.trenutna_postaja, kam, self.trenutni_cas)
        if rezultat:
            cas_odhoda, cas_prihoda = rezultat
            self.skupni_cas_cakanja += cas_odhoda - self.trenutni_cas
            self.trenutna_postaja = kam
            self.trenutni_cas = cas_prihoda

    #moja verzija - zdaj popravljena
    def premik(self, kam):
        kandidati = []
        for postaja, povezane_postaje in povezave.items():
            for ime_povezane_postaje, cas_odhoda_s_postaje, cas_prihoda_na_povezano in povezane_postaje:
                if postaja == self.trenutna_postaja and kam == ime_povezane_postaje:
                    if cas_odhoda_s_postaje >= self.trenutni_cas:
                        kandidati.append((cas_odhoda_s_postaje, cas_prihoda_na_povezano))

        if not kandidati:
            return

        cas_odhoda, cas_prihoda = min(kandidati)
        self.skupni_cas_cakanja += cas_odhoda - self.trenutni_cas
        self.trenutna_postaja = kam
        self.trenutni_cas = cas_prihoda
    ...
    def izguba(self):
        return self.skupni_cas_cakanja
