...
def uravnotezena(tovor):
    leva, desna = 0, 0
    for i, teza in enumerate(tovor):
        if i % 2 == 0:
            leva += teza
        else:
            desna += teza
    return abs(leva - desna) <= 10
...
def deli(paketi, kapacitete):
    ...
    for paket in paketi:
        kandidati = [i for i in range(len(ladje)) if ladje[i] + paket <= kapacitete[i]]
        if kandidati:
            izbrana = min(kandidati, key=lambda i: ladje[i])
            ladje[izbrana] += paket

    return ladje

def kontrola(ime):
    mnozica_preobremenjenih = set()
    with open(ime, "r", encoding="utf-8") as datoteka:
        for vrstica in datoteka:
            vsota = 0
            ime_ladje = vrstica.strip().split(": ")[0]
            nosilnost_ladje = vrstica.strip().split(": ")[1]
            teze_paketov = vrstica.strip().split(": ")[2].split(", ")
            for paket in teze_paketov:
                vsota += int(paket)
            if vsota > int(nosilnost_ladje):
                mnozica_preobremenjenih.add(ime_ladje)
    return mnozica_preobremenjenih

def pravilna(marsovec, hierarhija, antene):
    for martian in hierarhija.get(marsovec, []):
        if antene[martian] >= antene[marsovec]:
            return False
        if not pravilna(martian, hierarhija, antene):
            return False
    return True
...
class Ladja:
    def __init__(self):
        self.slovar_paradajzev = defaultdict(int) #slovar oblike tip_paradajza : st_paradajza

    def nalozi(self, sorta, teza):
        if sorta not in self.slovar_paradajzev and len(self.slovar_paradajzev) >= 3:
            return False
        self.slovar_paradajzev[sorta] += teza
        return True

    def kolicina(self, sorta):
        return self.slovar_paradajzev.get(sorta, 0)
