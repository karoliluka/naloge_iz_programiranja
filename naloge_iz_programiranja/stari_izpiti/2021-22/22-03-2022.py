...
def najvec_oken(krogi):
    ...
    centri_letal = letala(krogi)

    najvecje_stevilo = -1
    zmagovalci = []

    for krog in krogi:
        if krog[:2] in centri_letal:
            stevilo_oken = len(vsebuje[krog])
            if stevilo_oken > najvecje_stevilo:
                najvecje_stevilo = stevilo_oken
                zmagovalci = [krog[:2]]
            elif stevilo_oken == najvecje_stevilo:
                if krog[:2] not in zmagovalci:
                    zmagovalci.append(krog[:2])
...
class Strelec:
    def __init__(self):
        self.dodeljene_ladje = []

    def dodeli(self, krog):
        self.dodeljene_ladje.append(krog)

    def strel(self, x, y):
        zadetek = False
        for (cx, cy, r) in self.dodeljene_ladje.copy():
            if (x - cx)**2 + (y - cy)**2 < r**2:
                self.dodeljene_ladje.remove((cx, cy, r))
                zadetek = True
        return zadetek

    def preostalih(self):
        return len(self.dodeljene_ladje)
