...
def najmlajsi_v_rodbini(ime):
    if not otroci[ime]:
        return starost[ime], ime

    najmlajsi = starost[ime]
    for ime in otroci[ime]:
        age, name = najmlajsi_v_rodbini(ime)
        if age < najmlajsi:
            najmlajsi = age
            ime_najmlajsega = name
    return najmlajsi, ime_najmlajsega

def globina_do(od_ime, do_ime):
    if do_ime == od_ime:
        return 0

    for ime in otroci[od_ime]:
        if do_ime in imena_v_rodbini(ime):
            return globina_do(ime, do_ime) + 1
...
def pot_do(od_ime, do_ime):
    ...
    for ime in otroci[od_ime]:
        if do_ime in imena_v_rodbini(ime):
            seznam.extend(pot_do(ime, do_ime))
            return seznam
    return None
...
def dodaj_na_konec(s, x):
    if not s:
        return (x, ())
    return (s[0], dodaj_na_konec(s[1], x))

def reverse(s):
    if not s:
        return ()
    return dodaj_na_konec(reverse(s[1]), s[0])
