...
def razdalja(postaja1, postaja2):
    return razdalja_geo(postaje[postaja1]['latitude'], postaje[postaja1]['longitude'], postaje[postaja2]['latitude'], postaje[postaja2]['longitude'])

def najblizje3(postaja):
    mnozica_postaj = set()
    for station in postaje:
        if postaja != station:
            mnozica_postaj.add(station)

    seznam_razdalj = []
    for station in mnozica_postaj:
        seznam_razdalj.append((razdalja(postaja, station), station))

    return sorted(seznam_razdalj, key=lambda terka: terka[0])[:3]

def dosegljive(postaja, razdalja_max, otok):
    if postaja in otok:
        return
    ...
    for station in postaje:
        if station != postaja and razdalja(postaja, station) < razdalja_max:
            dosegljive(station, razdalja_max, otok)


def izolirana():
    slovar_razdalj = dict()
    mnozica_postaj = {postaja for postaja in postaje}
    for postaja in mnozica_postaj:
        slovar_razdalj[postaja] = najblizje3(postaja)[0]

    return max(slovar_razdalj, key=slovar_razdalj.get)
...
def preberi(ime_datoteke):
    ...
    with open(ime_datoteke, "r", encoding="utf-8") as datoteka:
        for vrstica in datoteka:
            ...
            st_koles, mesta = podatki.split("/")[0], podatki.split("/")[1]
            slovar[ime_postaje] = (st_koles, mesta)
    return slovar

class Koordinator:
    def __init__(self):
        self.koordinator_postaje = postaje

    def stanje(self, postaja):
        return self.koordinator_postaje[postaja]['bikes']

    def odpelji(self, postaja):
        if self.koordinator_postaje[postaja]['bikes'] >= 1:
            self.koordinator_postaje[postaja]['bikes'] -= 1
            return True
        return False

    def vrni(self, postaja):
        if self.koordinator_postaje[postaja]['bikes'] < self.koordinator_postaje[postaja]['capacity']:
            self.koordinator_postaje[postaja]['bikes'] += 1
            return True
        return False
