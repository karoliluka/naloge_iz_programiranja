...
def subs(niz, polozaj):
    return "".join(niz[int(p)] for p in polozaj)
...
def txt2morse(s):
    return " ".join([morse[char] for char in s])

obratni_slovar = {v: k for k, v in morse.items()}

def morse2txt(s):
    return "".join([obratni_slovar[beseda] for beseda in s.split(" ")])

def obrni(t):
    return [[vrstica[j] for vrstica in t] for j in range(len(t[0]))]
...
def valid(s):
    return len(s) == 10 and sum(i * vrednost(c) for i, c in enumerate(s, start=1)) % 11 == 0

def valid_ean(s):
    return (10 - (sum([int(x)*3 for i, x in enumerate(s[:-1], start=1) if i % 2 == 0])
                  + sum([int(x) for i, x in enumerate(s[:-1], start=1) if i % 2 == 1])) % 10) % 10 == int(s[-1])
